...
After do |scenario|
    ...
    image_name = "data/img/#{sufix}-#{name}.jpeg"
    temp_shot = page.save_screenshot(image_name)
    file_shot = File.open(temp_shot, "rb").read
    final_shot = Base64.encode64(file_shot)
    #def embed(temp_shot)
        #puts "Clique aqui para ver a evidência! #{temp_shot}"
     #end
    embed(temp_shot, "image/jpeg", "Clique aqui para ver a evidência!")
end
at_exit do
    time = Time.now
...
#    ReportBuilder.configure do |config|
#      config.input_path = "data/report.json"
#      config.report_path = "data/resultados"
#      config.report_types = [:html]
#      config.color = "indigo"
#      config.report_tabs = %w[Overview Features Scenarios Errors]
#      config.report_title = "Automação de Testes - Report"
#      config.compress_images = true
#      config.additional_info = { "Projeto" => "Sprint 2 QA", "Funcionalidade" => "Fluxo E2E", "Canal/Ambiente" => "Homolog", "Data de execução" => "#{data}" " #{hora}", "QA" => "Raphael Ribeira" }
#   end
#   ReportBuilder.build_report
# end